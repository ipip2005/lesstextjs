...
function normalizeOptions(options) {
    ...
    return __assign({ flexibleElement: options.monitorElement, lineHeight: lineHeight, linesCount: 2, omission: '...', omissionBreakLastWord: true, reserveSpace: false, separator: '' }, options);
}
...
export function truncate(rawOptions) {
    ...
    var wordsArray = scalableText
        ...
        .filter(function (text) { return options.reserveSpace || text; });
    ...
    var lastWord = wordsArray[minWordsCount];
    var currentText = wordsArray.slice(0, minWordsCount - 1).join(options.separator);
