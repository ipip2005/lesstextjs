...
function normalizeOptions(options) {
    ...
    return __assign({ flexibleElement: options.monitorElement, lineHeight: lineHeight, linesCount: 2, omission: '...', omissionBreakLastWord: false, reserveWhiteSpace: false, separator: '' }, options);
}
...
export function truncate(rawOptions) {
    ...
    var wordsArray = scalableText
        ...
        .filter(function (text) { return options.reserveWhiteSpace || text; });
    ...
    do {
        ...
        var experimentText = wordsArray.slice(0, experimentWordsCount).join(options.separator) + options.omission;
